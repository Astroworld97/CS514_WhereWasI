    ...
    private var visitsPreviewList: some View {
        VStack(spacing: f) {
            ForEach(visits[range]) { visit in
                VisitCell(visit: visit, size: sizeCategory.isAccessibilityCategory)
            }
        }
//        .onAppear(){
...
struct VisitsListView_Previews: PreviewProvider {
    static var previews: some View {
        DarkThemePreview {
            VisitsListView(visits: Visit.mocks(start: Date(), end: Date().addingTimeInterval(TimeInterval(60 * 60 * 24 * 2))), height: 300)
        }
    }
}
